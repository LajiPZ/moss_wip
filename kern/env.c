...
static int env_setup_vm(struct Env *e) {
	...
	e->env_pgdir[PDX(UVPT)] = PADDR(e->env_pgdir) | PTE_V | PTE_PLV | PTE_C;
	return 0;
}
 ...
 *     'env_id', 'env_asid', 'env_parent_id', 'env_tf.regs[3]', 'env_tf.cp0_status',
...
static int load_icode_mapper(void *data, u_long va, size_t offset, u_int perm, const void *src,
	...
	if (src != NULL) {
		...
		flush_cache(page2kva(p) + offset, len);
	}
...
static void load_icode(struct Env *e, const void *binary, size_t size) {
	...
	/* Step 3: Set 'e->env_tf.era' to 'ehdr->e_entry'. */
	/* Exercise 3.6: Your code here. */

	e->env_tf.era = ehdr->e_entry;
}
...
void env_run(struct Env *e) {
	...
	/* Step 4: Change 'PGDL' and 'PGDH' to 'cur_pgdir', which will be used in tlb refill. */
	...
	/* Step 5: Use 'env_pop_tf' to restore the curenv's saved context (registers) and return/go
	 * to user mode.
	 *
	 * Hint:
